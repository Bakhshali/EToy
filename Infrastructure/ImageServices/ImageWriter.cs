...
namespace Infrastructure.ImageServices
{
    public class ImageWriter : IImageWriter
    {
        ...
        public async Task<string> WriteFile(IFormFile file,string owner,string imgname)
        {
            try
            {
                string dir = Path.Combine(Directory.GetCurrentDirectory(), $"wwwroot\\assets\\images\\{owner}");
                if (!Directory.Exists(dir)) Directory.CreateDirectory(dir);
                string path = Path.Combine(Directory.GetCurrentDirectory(), $"wwwroot\\assets\\images\\{owner}", imgname);
        ...
        public string MoveFile(string owner, string oldname, string newname)
        {
            try
            {
                var path = Path.Combine(Directory.GetCurrentDirectory(), $"wwwroot\\assets\\images\\{owner}", oldname);
                var newpath = Path.Combine(Directory.GetCurrentDirectory(), $"wwwroot\\assets\\images\\{owner}", newname);
                if (File.Exists(path))
                {
