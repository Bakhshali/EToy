...
namespace EToy.Areas.Panel.Controllers
{
    ...
    public class ClothesCategoryController : MyController
    {
        [HttpGet]
        [Route("")]
        ...
        {
            var query = new GetAllCategoryQuery(page, 20);
            PaginatedList<CategoryOfClothes> result = await Mediator.Send(query);
            return View(result);
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Add(CreateCategoryCommand model)
        {
        ...
        public async Task<IActionResult> Update(Guid id)
        {
            ...
            GetCategoryVm result = await Mediator.Send(query);
            return View(result);
        }

        [HttpPost]
        public async Task<IActionResult> Update(Guid id, GetCategoryVm model)
        {
            var command = new UpdateCategoryCommand(id, model);
            OperationResult<GetCategoryVm> result = await Mediator.Send(command);
            if (result.Success)
            {
                return RedirectToAction(nameof(Index));
            }
        ...
        public async Task<IActionResult> CheckCategoryExist(string name)
        {
